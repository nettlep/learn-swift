 ...
 * Ternary operators work on three targets. There is only one ternary: `a ? b : c`.
 ...
 not actually return a value, so the C-like statment `if (x = y) {}` will not work. This helps prevent
...
/*:
 ...
 (`%`) which is not to be confused with the standard modulo. They work differently because of the way remainders
 are calculated for negative numbers.  Negative remainders can always be converted to modulo by adding the modulus.

 Remainder ONLY works on Integers, not on floating point
 */
var c = a / b
...
/*:
 Note that you can have a Range operator on things other than `Int`'s.
 */
let stringRange = "a" ... "z"
/*: 
 But that the following will not compile
```
 for s in stringRange {
    // do something.
 }
```
 Because stringRange is not a `CountableRange`
...
a
/*:
 Binary operators are infix because they appear between two targets
*/
a + b
/*:
