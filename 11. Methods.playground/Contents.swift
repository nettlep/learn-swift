...
class Counter
{
	...
	// Two parameters. Since _ external name is specified on first, caller
	// need not specify the first parameter's external name, but must specify all others:
	func addTwiceWithExternalImplied(_ first: Int, second: Int) {
		count += first
		count += second
	}
	...
	func addTwiceWithExternalSpecified2(_ first: Int, _ second: Int) {
		count += first
		count += second
	}
}
...
counter.addTwiceWithExternalSpecified2(10, 10)
counter.count
/*:
 The 'self' property refers to the current instance of a class, structure or enumeration. For
 C++ or JavaScript developers, think of 'self' as 'this'.
*/
class Point {
	var x: Int = 10
