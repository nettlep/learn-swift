...
/*:
 ...
 1. The calls to map can't return arrays, they have to return some other
 type that knows how to do some processing and pass the result to the
 next thing in the chain.  We call these things Publishers.
 
 2. We need some way to know that we're done.  Array knows it's done
 because it knows how many things there are in the array when we call map.
 ...
 3. We need someway to handle errors. There aren't many choices here,
 you sort of have to pass the error down the chain.
 
 4. Something needs to sit at the end of the chain and process the things
 ...
 5. For long running Sequences, we'd like the opportunity to cancel the
 ...
 how each type of Publisher works (Map, FlatMap, Filter, etc are all
 different _types_ of publisher, not just different functions).
 ...
 to `compose` a function from a) itself and b) a function presented
 ...
 Read that paragraph again.  We did not talk about functional
 ...
 recursively.  You can also just try to understand Combine holistically,
...
/*:
 ...
 5 above. In this case everything is done before we could have a chance to
...
So if you go and look at the Apple doc on PassthruSubject, you will find the following:
 ...
 ```
 So this is way we can interface our existing imperative code to the functional world.  When our imperative code generates
...
/*:
 ...
 In the next playground we'll do an example of network access using Combine and
 that will finally show how we deal with requirement 3 above (error handling)
*/
