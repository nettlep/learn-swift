...
/*:
 Classes are a little different in that we have to deal with inheritance.  You would like a class
 to be able to override properties of its base class. This can be done ONLY if the property is a computed
 property and is done by using the `class var` or `class let` construct
*/
class SomeClass {
    static var howdy = "Howdy!"
    // The following line won't compile because classes aren't allowed to declare class variables of stored type
	
    // class var storedTypeProperty = "some value"
	
	// This does compile.  This is read-only, but you can also do read/write
	class var computedTypeProperty: Int { return 4 }
}

class SomeOtherClass: SomeClass {
    // This line does not compile because classes cannot override stored type variables of their base class
    
    // override static var howdy = "Nope."
    
    // This does compile because classes can override class variables of computed type
    override class var computedTypeProperty: Int { return 5 }
}
/*:
...
SomeOtherClass.computedTypeProperty
/*:
 You can dynamically determine the type and then invoke the computed property on the type you discovered
 */
let someInstance = SomeClass()
type(of: someInstance).computedTypeProperty

let someOtherInstance = SomeOtherClass()
type(of: someOtherInstance).computedTypeProperty
